...
def resolve(node, facts):
    ...
    #     node.resolved_value = facts.get(node.value)  # True si connu, sinon False
    #     return node.resolved_value
    
    # Si c'est un fait atomique (A, B, C, ...)
    if node.value.isalnum():
        node.resolved_value = facts.get(node.value, None)  # None si non trouvé
        return node.resolved_value
    ...
    if node.value == "<=>":
        ...
        # Si un côté est indéterminé, propager la valeur de l'autre côté s'il est connu
...
#         if left:  # Si le côté gauche est vrai, le côté droit doit être vrai
#             node.resolved_value = right
#             if right:  # Si la conclusion est vraie, mettre à jour le fait
#                 node.right.resolved_value = True
#                 facts[node.right.value] = True
#         else:  # Si le côté gauche est faux, l'implication est toujours vraie
#             node.resolved_value = True
...
#         node.resolved_value = left == right  # Équivalence logique

#         # Propagation de la valeur dans les deux sens
#         if left is True:
#             node.right.resolved_value = True
#             facts[node.right.value] = True
#         elif right is True:
